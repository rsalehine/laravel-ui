@if (count($data) === 1)
    <livewire:single-code-block :data="$data[0]" />
@else
    <div data-tab-fx-site x-tabs
        ...
        <div data-panels-container
            class="relative group overflow-hidden max-h-[35rem] bg-(--astro-code-color-background) border border-gray-700 dark:border-gray-800 inner-radius w-full grid">
            @foreach ($data as $item)
                @php
                    $id = Str::slug($item['name']);
                @endphp
